...
class S3Utils:
    ...
    def get_s3_list_default(self):
        s3_bucket = os.environ.get('S3_BUCKET_DEMO')
        if s3_bucket is None or s3_bucket=="":
            return ["ERROR: env variable S3_BUCKET_DEMO is required"]
    ...
    def get_s3_file_default(self):
        s3_bucket = os.environ.get('S3_BUCKET_DEMO')
        s3_file = os.environ.get('S3_FILE_DEMO')
        if s3_bucket is None or s3_bucket==""  or  s3_file is None or s3_file=="":
            return "ERROR: env variable S3_BUCKET_DEMO and S3_FILE_DEMO are required"
        return self.get_s3_file(s3_file, s3_bucket)

    def get_s3_file(self, s3_file, s3_bucket ):
        if s3_bucket is None or s3_bucket=="" or  s3_file is None or s3_file=="":
            return "ERROR: parameter s3_bucket and s3_file are required"
        conn = boto.connect_s3()
        bucket = conn.get_bucket(s3_bucket)
